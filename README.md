...
The DL model generates heatmaps (HMs) and pair affinity fields (PAFs) form the imput image. The HMs and PAFs are a kind of image in 57x32 resolution, and the HMs consists of 18 images and the PAFs consists of 18x2 images.  
...
This project contains a small script file to copy and build the human-pose `pose_extractor` and a Python demo program which demonstrates how to use this `pose_extractor` form a Python code.  
...
このプロジェクトはこの`pose_extractor`後処理モジュールをビルドするための小さなスクリプトと、このモジュールを利用する方法を示すためのPythonコードからなります。
